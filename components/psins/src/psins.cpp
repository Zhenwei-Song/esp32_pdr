...
const CVect3 O31(0.0), One31(1.0), Ipos(1.0 / RE, 1.0 / RE, 1.0), posNWPU = LLH(23.305836, 113.572256, 22); // NWPU-Lab-pos
...
void CSINSGNSS::SetFt(int nnq)
{
    ...
    if (nnq >= 34) {
        ...
        Ft.SetMat3(3, 28, Cfx);
        Ft.SetMat3(3, 31, Cfz); // 28-33 dKA(xx,yx,zx, yy,zy, zz)
    }
}
...
CSINSGNSSOD::CSINSGNSSOD(double ts) : CSINSGNSS(18, 17, ts)
{
    ...
    odmeast = 0.0;
    odmeasT = 1.0;
    gnsslostdist = distance = distlost = 0.0;
...
void CSINSGNSSOD::SetMeasOD(double dS)
{
    ...
    if (odmeast >= odmeasT) { // take measurement every (odmeasT)(s)
        ...
        Hk.SetMat3(6, 15, CMat3(IMv.e02, -IMv.e01, -IMv.e00, IMv.e12, -IMv.e11, -IMv.e10, IMv.e22, -IMv.e21, -IMv.e20));
...
int CAVPInterp::Interp(double tpast)
{
    ...
    if (tleft > 0.99 * ts) {
        ...
        if (normInf(att - atti[k1]) > 10.0 * DEG || fabs(pos.j - posi[k1].j) > 1.0 * DEG)
            res = 0;
    }
    return res;
}
...
void CMahony::Update(const CVect3 &wm, const CVect3 &vm, double ts, const CVect3 &mag)
{
    ...
    if (exyzInt.k > ebMax.k)
        ...
        exyzInt.k = -ebMax.k;
#ifdef DEBUG
    printf("exyzInt.i:%f, exyzInt.j:%f, exyzInt.k:%f\n", exyzInt.i, exyzInt.j, exyzInt.k);
#endif
}
...
void CQEAHRS::Update(const CVect3 &gyro, const CVect3 &acc, const CVect3 &mag, double ts)
{
    ...
    // Ft
    Ft.dd[1] = 0, Ft.dd[1] = -wx / 2, Ft.dd[2] = -wy / 2, Ft.dd[3] = -wz / 2, Ft.dd[4] = q1 / 2, Ft.dd[5] = q2 / 2, Ft.dd[6] = q3 / 2;
...
void CFileRdWt::Dir(const char *dirI, const char *dirO) // set dir
{
    ...
    if (_access(dirIn, 0) == -1)
        dirIn[0] = '\0';
    if (_access(dirOut, 0) == -1)
        dirOut[0] = '\0';
}
...
void CConUart::dispUart(int itv)
{
    ...
    printf("\n\tGPS״̬(N.PDOP)\t%10.3f\t \n\tGPSʱ��(ms)\t%10.3f\t", ps->gpsstat / 1000, ps->gpsdly * 1000);
    printf("\n\n\t�¶�(��)\t%10.3f\t", ps->tmp);
...
CVect3 randn(const CVect3 &mu, const CVect3 &sigma)
{
    CVect3 vtmp;
    double *p = &vtmp.i;
    const double *pmu = &mu.i, *psigma = &sigma.i;
    for (int i = 0; i < 3; i++, p++, pmu++, psigma++)
        *p = randn(*pmu, *psigma);
    return vtmp;
}
...
CVect randn(const CVect &mu, const CVect &sigma)
{
    ...
    double *p = &vtmp.dd[0];
    const double *pmu = &mu.dd[0], *psigma = &sigma.dd[0];
    for (int i = 0; i < vtmp.rc; i++, p++, pmu++, psigma++)
        *p = randn(*pmu, *psigma);
    return vtmp;
}
...
CMat3 randn(const CMat3 &mu, const CMat3 &sigma)
{
    CMat3 mtmp;
    double *p = &mtmp.e00;
    const double *pmu = &mu.e00, *psigma = &sigma.e00;
    for (int i = 0; i < 9; i++, p++, pmu++, psigma++)
        *p = randn(*pmu, *psigma);
    return mtmp;
}
