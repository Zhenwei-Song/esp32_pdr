...
#define stackstart()                \
    {                               \
        int stack0 = 0;             \
        psinsstack0 = (int)&stack0; \
    }
#define stacksize()                                                       \
    {                                                                     \
        int stack1 = 0;                                                   \
        psinsstacksize = max(psinsstacksize, psinsstack0 - (int)&stack1); \
    }
#else
...
public:
    ...
    // friend BOOL IsZeroXY(const CVect3 &v, double eps = EPS);                                                     //
    ...
    friend CVect3 pow(const CVect3 &v, int k);
...
CVect3 pp2vn(const CVect3 &pos1, const CVect3 &pos0, double ts = 1.0, CEarth *pEth = (CEarth *)NULL); // position difference to velocity
...
public:
    ...
    friend CMat3 sfoam(const CMat3 &B, int iter);
...
CMat3 randn(const CMat3 &mu, const CMat3 &sigma = One33);
...
public:
    ...
    CQuat qnbE;
    CVect3 attE, vnE, posE; // for extrapolation
    CMaxMin mmwb, mmfb;
