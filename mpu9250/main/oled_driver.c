...
esp_err_t oled_init()
{
    ...
    esp_err = i2c_param_config(OLED_I2C_POER_NUM, &conf);
    // printf("i2c_param_config: %d \n", esp_err);

    esp_err = i2c_driver_install(OLED_I2C_POER_NUM, I2C_MODE_MASTER, 0, 0, 0);
    // printf("i2c_driver_install: %d \n", esp_err);

    esp_err = i2c_master_write_slave(OLED_I2C_POER_NUM, oled_init_cmd, 56);
    // printf("init command write: %d \n", esp_err); 
